...
import { X, User, Home, SortAsc, SortDesc, Hash } from 'lucide-react';

export type AccommodationSortField = 'name' | 'ref' | 'city' | 'address' | 'workers';
export type SortDirection = 'asc' | 'desc';

export interface AccommodationSortConfig {
  field: AccommodationSortField;
  direction: SortDirection;
}

interface AccommodationSortModalProps {
  isOpen: boolean;
  onClose: () => void;
  sortConfig: AccommodationSortConfig;
  onApply: (config: AccommodationSortConfig) => void;
}

const AccommodationSortModal: React.FC<AccommodationSortModalProps> = ({ isOpen, onClose, sortConfig, onApply }) => {
  const updateSort = (updates: Partial<AccommodationSortConfig>) => {
    onApply({ ...sortConfig, ...updates });
  };

  const fields = [
    { id: 'name' as const, label: 'Nombre', icon: <Home size={14} /> },
    { id: 'ref' as const, label: 'Referencia', icon: <Hash size={14} /> },
    { id: 'city' as const, label: 'Ciudad', icon: <Hash size={14} /> },
    { id: 'address' as const, label: 'Dirección', icon: <Hash size={14} /> },
    { id: 'workers' as const, label: 'Operarios', icon: <User size={14} /> },
  ];
    ...
    <>
      ...
      <div className={`absolute top-full right-0 mt-3 z-[110] bg-white/90 dark:bg-stone-900/95 backdrop-blur-xl w-[320px] rounded-2xl overflow-hidden border border-white/60 dark:border-stone-800/50 soft-shadow transition-all duration-300 ease-out origin-top-right ${
        ...
        <div className="px-5 py-4 flex items-center justify-between border-b border-stone-100 dark:border-stone-800/50">
          <h2 className="text-base font-normal text-slate-800 dark:text-stone-200 font-display tracking-tight">Ordenar Alojamientos</h2>
          <button 
            onClick={onClose}
...
export default AccommodationSortModal;
