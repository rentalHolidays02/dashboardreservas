...
export default function Home() {
  ...
  const URL_CSV = "const SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQ9XN1hs2aSSFFCI9EOP4zCo_RILZY-SXIok-ourvii_sx64LzJsY3T-AGPcllJBUjFqcuMk0UcAhpQ/pub?output=csv";
    ...
    try {
      ...
      Papa.parse(textoCSV, {
        ...
        complete: (resultado) => {
          ...
          let idxCabecera = filas.findIndex(f => f.some(c => String(c).toUpperCase().includes("ALOJAMIENTO")));
          if (idxCabecera === -1) idxCabecera = 0;

          const cabeceras = filas[idxCabecera].map(c => String(c).trim().toUpperCase());
          
          // Mapeo Inteligente para el Excel 2026
          const idxRef = cabeceras.findIndex(c => c.includes("REF") || c === "`");
          const idxPropiedad = cabeceras.findIndex(c => c.includes("ALOJAMIENTO"));
          ...
          for (let i = idxCabecera + 1; i < filas.length; i++) {
            ...
            const alojamiento = String(fila[idxPropiedad]).trim();
            if (!alojamiento || alojamiento.toUpperCase() === "ALOJAMIENTO") continue;

            registros.push({
              ref: idxRef > -1 ? fila[idxRef] : '-',
          ...
          // Asignar las keys estáticas para que tu JSX las entienda
          setKeys({
            alojamiento: 'alojamiento',
            origen: 'origen',
            entrada: 'entrada',
          ...
          const enriched = registros.map((row, i) => ({
            ...row,
            _id: i,
            _entrada: parseDate(row.entrada),
            _salida: parseDate(row.salida),
            _origen: (row.origen || '').toLowerCase(),
          }));
